...
class FileDirectInstaller:
    ...
    def _check_model_exists_in_path(self, model_data: Dict) -> bool:
        """Check if model exists in the specific save_path from model_data configuration."""
        try:
            model_name = model_data.get("filename", "")
            if not model_name:
                return False

            # Handle HuggingFace placeholder
            if model_name == HUGGINGFACE_PLACEHOLDER:
            ...
            # For save_path == "default", also check all configured directories
            if save_path == "default":
                model_dir_name = self.MODEL_DIR_NAME_MAP.get(model_type.lower())
                if model_dir_name is not None:
                    try:
                        ...
                        for dir_path in dirs:
                            check_path = os.path.join(dir_path, model_name)
                            if (
    ...
    @staticmethod
    def _get_model_dir_static(save_path: str, model_type: str) -> str:
        """Static version of _get_model_dir for use in static methods."""
        if "download_model_base" in folder_paths.folder_names_and_paths:
            models_base = folder_paths.folder_names_and_paths["download_model_base"][0][
                0
            ]
    ...
    def check_model_installed(json_obj):
        """Check if models are installed based on their save_path configuration."""

        def check_model_exists(item):
            """Check if a single model exists in its configured save_path."""
            try:
                model_name = item.get("filename", "")
                if not model_name:
                ...
                save_path = item.get("save_path", "default")
                model_type = item.get("type", "checkpoints")
                model_dir = FileDirectInstaller._get_model_dir_static(
                    save_path, model_type
                )
                model_path = os.path.join(model_dir, model_name)

                # Check if file exists in the specific save_path
                if os.path.exists(model_path) and os.path.getsize(model_path) > 0:
                    item["installed"] = "True"
                    return
        ...
        with concurrent.futures.ThreadPoolExecutor(MAX_WORKERS) as executor:
            for item in json_obj["models"]:
                executor.submit(check_model_exists, item)
    ...
    async def get_installed_nodes(self) -> Set[str]:
        try:
            ...
            for node_id, node_info in self._cached_node_packs.items():
                ...
                aux_id = node_info.get("aux_id", "")
                if aux_id:
                    repo_name = aux_id.split("/")[-1]
                    nodes.update(self._normalize_node_id(repo_name))
                    nodes.update(self._normalize_node_id(aux_id))
            return nodes
    ...
    async def uninstall_custom_node(self, node_data: Dict) -> bool:
        try:
            node_id = node_data.get("id") or node_data.get("file_name")
            if not node_id and node_data.get("files"):
                node_id = node_data["files"][0].split("/")[-1].replace(".git", "")
    ...
    async def install_model(self, model_data: Dict) -> bool:
        try:
            ...
            # Check if model already exists in the specified save_path
            model_path = os.path.join(model_dir, model_name)
            if os.path.exists(model_path) and os.path.getsize(model_path) > 0:
                logger.debug(
                    f"Model already exists at {model_path}, skipping installation: {model_name}"
                )
                if model_name not in self.installed_models:
                    self.installed_models.add(model_name)
                return False

            logger.info(f"Starting model download: {model_name} to {model_dir}")
        ...
        try:
            ...
            for model_data in data.get("models", []):
                try:
                    ...
                    # Check if model exists in the specific save_path from configuration
                    try:
                        is_installed = self._check_model_exists_in_path(model_data)
                    except Exception as e:
                        logger.warning(
                    ...
                    if is_installed:
                        ...
                        save_path = model_data.get("save_path", "default")
                        logger.debug(
                            f"Model already exists at save_path '{save_path}', skipping installation: {model_name}"
                        )
                        continue

                    save_path = model_data.get("save_path", "default")
                    logger.info(
                        f"Model {model_name} not found at save_path '{save_path}', starting download..."
                    )
    ...
    async def execute_restart_command(self):
        if not self.restart_command:
            logger.info("No restart command configured, skipping restart")
